...
public class WritingDelayTestCase {
    ...
    private HttpServerOperationBuilderContext startHttpEmulator() {
        return Emulator.getHttpEmulator().server()
                .given(HttpServerConfigBuilderContext.configure().host("127.0.0.1")
                        .port(6065).context("/users").withWritingDelay(1000))

                .when(HttpServerRequestBuilderContext.request().withMethod(HttpMethod.GET).withPath("/user1"))
                .then(HttpServerResponseBuilderContext.response().withBody("User1")
                        .withStatusCode(HttpResponseStatus.OK).withHeader("Header1", "value1"))

                .when(HttpServerRequestBuilderContext.request().withMethod(HttpMethod.PUT).withPath("/user2"))
                .then(HttpServerResponseBuilderContext.response().withBody("User2")
                        .withStatusCode(HttpResponseStatus.OK).withHeader("Header2", "value2"))

                .when(HttpServerRequestBuilderContext.request().withMethod(HttpMethod.POST).withPath("user3"))
                .then(HttpServerResponseBuilderContext.response().withBody("User3")
